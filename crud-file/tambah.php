...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['task_file']) && $_FILES['task_file']['error'] === UPLOAD_ERR_OK) {
        ...
        if (in_array($mime, $allowed_types, true)) {
            ...
            if (move_uploaded_file($_FILES['task_file']['tmp_name'], $filepath)) {
                ...
                $file_original_name = $conn->real_escape_string($_FILES['task_file']['name']);
            }
        }
    }
    ...
    if ($mode === 'tambah') {
        ...
        $stmt->bind_param('issssssss', $id_user, $title, $description, $priority, $activity_date, $deadline, $label, $file_attachment, $file_original_name);
    } else {
        if ($file_attachment !== null) {
            ...
            $stmt->bind_param('sssssss sii', $title, $description, $priority, $activity_date, $deadline, $label, $file_attachment, $file_original_name, $id, $id_user);
        } else {
            $stmt = $conn->prepare(
                "UPDATE daily_activities
    ...
    <div class="form-card">
      <form method="POST" enctype="multipart/form-data">
        ...
        <div class="form-group">
          ...
          <input type="file" name="task_file" accept=".pdf,.doc,.docx,.jpg,.jpeg,.png,.gif,.webp,.zip,.txt">
        </div>
        ...
        <div class="form-grid">
          ...
          <div class="form-group">
            ...
            <select name="label" id="label" onchange="cekLabel(this.value)">
              ...
              <option value="sekolah"   <?= $tugas && $tugas['label'] === 'sekolah'   ? 'selected' : '' ?>>Sekolah</option>
...
const pilihanDefault = ['personal', 'sekolah', 'kerja', 'lainnya'];
